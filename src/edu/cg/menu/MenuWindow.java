...
public class MenuWindow extends JFrame implements Logger {
	...
	public void removeObjectFromImage(boolean[][] srcMask) {
		...
		int outWidth = scaleSelector.width();
		RGBWeights rgbWeights = colorMixer.getRGBWeights();
		int numOfSeams = Math.min(getNumberOfTrueValues(srcMask), (width / 3) - 1);
		SeamsCarver remove = new SeamsCarver(this, duplicateImage(), width - numOfSeams, rgbWeights, srcMask);
		BufferedImage img;

		img = remove.resize();
		SeamsCarver increase = new SeamsCarver(this, img, outWidth, rgbWeights, remove.getMaskAfterSeamCarving());
		img = increase.resize();
		present(img, "Image After Object Removal");
	}

	private int getNumberOfTrueValues(boolean[][] srcMask){
		int trueValues = 0;
		...
		for (int i = 0; i < srcMask.length; i++) {
			...
			trueValues = Math.max(total, trueValues);
		}
